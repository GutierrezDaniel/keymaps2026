...
| Tauri boundary | Typed `create_vault`, `unlock`, `lock`, `list`, `list_emails`, `get_entry_details`, `create`, `update`, `delete`, `export`, `copy_field`, and `record_activity` commands (12 total) | Direct React database access, broad JSON | Keeps policy and secrets in Rust and makes IPC contracts testable. `create_vault` was added to the original 10 (the MVP flow starts with vault creation) and `list_emails` powers the email filter's distinct-email selector. |
